...
────────────────────────────────────────────── */
const CHAT_MESSAGES = [
  { type: "bot", text: "Olá! 💆‍♀️ Bem-vinda à Clínica Bella. Sou a Luna, sua assistente. Como posso te ajudar hoje?" },
  { type: "user", text: "Oi! Quero agendar uma limpeza de pele" },
  { type: "bot", text: "Perfeito! Tenho horários disponíveis essa semana. Você prefere manhã ou tarde? ☀️" },
  { type: "user", text: "Quarta de manhã se tiver" },
  { type: "bot", text: "✅ Agendado! Quarta às 10h — Limpeza de pele (60min). Vou te lembrar na véspera. Até lá! ✨" },
];
...
function ChatDemo() {
  ...
  useEffect(() => {
    ...
    const delay = CHAT_MESSAGES[visible].type === "bot" ? 900 : 600;
    setTyping(CHAT_MESSAGES[visible].type === "bot");
    const t1 = setTimeout(() => setTyping(false), delay - 200);
    const t2 = setTimeout(() => setVisible(v => v + 1), delay + 400);
    return () => { clearTimeout(t1); clearTimeout(t2); };
  }, [visible]);
  return (
    <div className="hero-visual">
      <div style={{marginBottom:12,display:"flex",alignItems:"center",gap:10}}>
        <div style={{width:36,height:36,borderRadius:"50%",background:"linear-gradient(135deg,#F2D4CF,#C9847A)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>💆‍♀️</div>
        <div>
          ...
          <div style={{fontSize:11,color:"#C9847A",display:"flex",alignItems:"center",gap:4}}>
            <span style={{width:6,height:6,background:"#4cd964",borderRadius:"50%",display:"inline-block"}}></span> Online agora
          </div>
        </div>
      </div>
      {CHAT_MESSAGES.slice(0, visible).map((m, i) => (
        <div key={i}>
          <div style={{fontSize:11,color:"rgba(255,255,255,.4)",marginBottom:3,textAlign:m.type==="user"?"right":"left"}}>
            {m.type==="user"?"Você":"Luna"}
          </div>
          <div className={`chat-bubble ${m.type}`} style={{animationDelay:`${i*.05}s`}}>{m.text}</div>
        </div>
      ))}
