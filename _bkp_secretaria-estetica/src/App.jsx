...
/* ══════════════════════════════════════════════
   SECRETARIA PAGE — edição inline
══════════════════════════════════════════════ */
function SecretariaPage(){
  const[editing,setEditing]=useState(null); // null | 'assistente' | 'servicos'
  ...
  const addSvc=()=>setServices(s=>[...s,{id:Date.now(),name:"",price:"",duration:"60"}]);
  const rmSvc=(id)=>setServices(s=>s.filter(x=>x.id!==id));
  const updSvc=(id,f,v)=>setServices(s=>s.map(x=>x.id===id?{...x,[f]:v}:x));

  const save=()=>{setSaved(true);setEditing(null);setTimeout(()=>setSaved(false),3000);};

  const preview=`Olá! ${emoji} Bem-vinda à Clínica Bella. Sou ${nome}, sua assistente. Posso agendar serviços, tirar dúvidas ou te conectar com nossa atendente. 😊`;

  return(
    <>
      <div className="dash-topbar">
        <div><h1 className="dash-title">Minha Secretária</h1><p className="dash-subtitle">Configuração ativa da sua assistente virtual.</p></div>
        {saved&&<div style={{background:"rgba(52,199,89,.12)",border:"1px solid rgba(52,199,89,.3)",borderRadius:100,padding:"8px 18px",fontSize:13,color:"#1a8a3c",fontWeight:600}}>✓ Salvo com sucesso!</div>}
      </div>

      {/* Preview da assistente */}
      <div className="dash-section" style={{marginBottom:16}}>
        <div className="dash-section-header">
          <span className="dash-section-title">Preview da assistente</span>
          <button className="dash-edit-btn" onClick={()=>setEditing(editing==="assistente"?null:"assistente")}>
            {editing==="assistente"?"✕ Fechar":"✏️ Editar"}
          </button>
        </div>
        <div className="assistant-preview" style={{marginBottom:editing==="assistente"?16:0}}>
          <div className="assistant-header">
            <div className="assistant-avatar">{emoji}</div>
            <div><div className="assistant-name">{nome}</div><div className="assistant-status">Ativa e respondendo</div></div>
          </div>
          <div className="preview-bubble">{preview}</div>
        </div>
        ...
        {editing==="assistente"&&(
          <div style={{borderTop:"1px solid rgba(0,0,0,.07)",paddingTop:16}}>
            <div className="field-row">
              <div className="field">
                <label>Nome da assistente</label>
                <input type="text" value={nome} onChange={e=>setNome(e.target.value)} placeholder="Ex: Luna, Sofia..."/>
              </div>
              <div className="field">
                <label>Emoji</label>
                <input type="text" value={emoji} onChange={e=>setEmoji(e.target.value)} placeholder="💆‍♀️"/>
              </div>
            </div>
            <div className="field">
              <label>Tom de atendimento</label>
            ...
            <div className="field">
              ...
              <div className="field-hint">A opção de falar com a atendente é sempre incluída automaticamente.</div>
            </div>
            <div style={{display:"flex",gap:10}}>
              <button className="btn-next primary" style={{padding:"10px 28px"}} onClick={save}>Salvar assistente</button>
      ...
      <div className="dash-section">
        <div className="dash-section-header">
          <span className="dash-section-title">Serviços cadastrados</span>
          <button className="dash-edit-btn" onClick={()=>setEditing(editing==="servicos"?null:"servicos")}>
            {editing==="servicos"?"✕ Fechar":"✏️ Editar"}
          </button>
        </div>

        {editing!=="servicos"?(
          <>
            <div style={{display:"grid",gridTemplateColumns:"1fr 100px 80px",gap:8,padding:"0 0 8px",borderBottom:"1px solid rgba(0,0,0,.06)",marginBottom:8}}>
              {["Tipo de Serviço","Valor","Duração"].map((h,i)=>(
                <span key={i} style={{fontSize:11,fontWeight:700,color:"var(--muted)",textTransform:"uppercase",letterSpacing:.5,textAlign:i>0?"right":"left"}}>{h}</span>
              ))}
            </div>
            {services.map(s=>(
              <div key={s.id} style={{display:"grid",gridTemplateColumns:"1fr 100px 80px",gap:8,padding:"10px 0",borderBottom:"1px solid rgba(0,0,0,.04)",fontSize:14,color:"var(--mid)"}}>
                <span style={{fontWeight:500,color:"var(--charcoal)"}}>{s.name||"—"}</span>
                <span style={{textAlign:"right"}}>R$ {s.price||"0"}</span>
                <span style={{textAlign:"right"}}>{s.duration||"60"} min</span>
              </div>
            ))}
          </>
        ):(
          <>
            <div style={{display:"grid",gridTemplateColumns:"1fr 100px 80px 32px",gap:8,padding:"0 0 6px"}}>
              {["Tipo de Serviço","Valor (R$)","Duração",""].map((h,i)=>(
                <span key={i} style={{fontSize:11,fontWeight:700,color:"var(--muted)",textTransform:"uppercase",letterSpacing:.5,textAlign:i===1||i===2?"right":"left"}}>{h}</span>
              ))}
            </div>
            <div className="service-list">
              {services.map(s=>(
                <div key={s.id} className="service-item" style={{gridTemplateColumns:"1fr 100px 80px 32px"}}>
                  <input type="text" placeholder="Nome do serviço" value={s.name} onChange={e=>updSvc(s.id,"name",e.target.value)} style={{minWidth:0}}/>
                  <div style={{display:"flex",alignItems:"center",gap:3}}>
                    <span style={{fontSize:11,color:"var(--muted)",flexShrink:0}}>R$</span>
                    <input type="number" placeholder="0" value={s.price} onChange={e=>updSvc(s.id,"price",e.target.value)} style={{width:"100%",textAlign:"right"}}/>
                  </div>
            ...
            <div style={{display:"flex",gap:10,marginTop:16}}>
              <button className="btn-next primary" style={{padding:"10px 28px"}} onClick={save}>Salvar serviços</button>
              <button className="btn-back" onClick={()=>setEditing(null)}>Cancelar</button>
            </div>
          </>
        )}
      ...
      {/* Resumo de outras configs */}
      <div className="dash-section">
        <div className="dash-section-title" style={{marginBottom:12}}>Outras configurações</div>
        {[["Plano ativo","Profissional — R$ 197/mês"],["Horário de atendimento","Seg–Sex | 09:00–18:00"],["WhatsApp","(11) 99999-0000"],["Falar com humano","Sempre disponível nas conversas"]].map(([l,v])=>(
          <div key={l} className="config-row"><span className="config-label">{l}</span><span className="config-value">{v}</span></div>
        ))}
        <div style={{marginTop:14,fontSize:13,color:"var(--muted)"}}>Para editar horários acesse <strong>Agenda → Editar Horários</strong>. Para plano e cobrança acesse <strong>Configurações</strong>.</div>
      </div>
    </>
  );
}
...
/* ══════════════════════════════════════════════
   CONFIG PAGE — edição inline por seção
══════════════════════════════════════════════ */
function ConfigPage(){
  const[editing,setEditing]=useState(null); // null | 'clinica' | 'assistente_cfg' | 'cobranca'
  const[saved,setSaved]=useState(null);

  // Dados da clínica
  const[clinNome,setClnNome]=useState("Clínica Bella Estética");
  const[clinWpp,setClnWpp]=useState("(11) 99999-0000");
  const[clinCidade,setClnCidade]=useState("São Paulo, SP");
  const[clinEmail,setClnEmail]=useState("bella@clinica.com.br");

  // Assistente
  const[asstNome,setAsstNome]=useState("Luna");
  ...
  const stSenha=passwordStrength(senhaNova);

  const save=(section)=>{setSaved(section);setEditing(null);setTimeout(()=>setSaved(null),3000);};

  const tomLabels={amigavel:"Amigável e descontraído 😊",profissional:"Profissional e elegante 💼",carinhoso:"Carinhoso e próximo 💕",objetivo:"Direto e objetivo ⚡"};

  return(
    <>
      <div className="dash-topbar">
        <div><h1 className="dash-title">Configurações</h1><p className="dash-subtitle">Edite os dados da sua conta e secretária.</p></div>
        {saved&&<div style={{background:"rgba(52,199,89,.12)",border:"1px solid rgba(52,199,89,.3)",borderRadius:100,padding:"8px 18px",fontSize:13,color:"#1a8a3c",fontWeight:600}}>✓ {saved} salvo!</div>}
      </div>

      {/* DADOS DA CLÍNICA */}
      <div className="dash-section" style={{marginBottom:16}}>
        <div className="dash-section-header">
          <span className="dash-section-title">Dados da clínica</span>
          <button className="dash-edit-btn" onClick={()=>setEditing(editing==="clinica"?null:"clinica")}>
            {editing==="clinica"?"✕ Fechar":"✏️ Editar"}
          </button>
        </div>
        {editing!=="clinica"?(
          <>
            {[["Nome",clinNome],["WhatsApp",clinWpp],["Cidade",clinCidade],["E-mail",clinEmail]].map(([l,v])=>(
              <div key={l} className="config-row"><span className="config-label">{l}</span><span className="config-value">{v}</span></div>
            ))}
          </>
        ):(
          <>
            <div className="field"><label>Nome da clínica</label><input type="text" value={clinNome} onChange={e=>setClnNome(e.target.value)}/></div>
            <div className="field-row">
              <div className="field"><label>WhatsApp</label><input type="text" value={clinWpp} onChange={e=>setClnWpp(e.target.value)}/></div>
              <div className="field"><label>Cidade</label><input type="text" value={clinCidade} onChange={e=>setClnCidade(e.target.value)}/></div>
            </div>
            <div className="field"><label>E-mail</label><input type="email" value={clinEmail} onChange={e=>setClnEmail(e.target.value)}/></div>
            <div style={{display:"flex",gap:10}}>
      ...
      <div className="dash-section" style={{marginBottom:16}}>
        <div className="dash-section-header">
          <span className="dash-section-title">Configuração da assistente</span>
          <button className="dash-edit-btn" onClick={()=>setEditing(editing==="assistente_cfg"?null:"assistente_cfg")}>
            {editing==="assistente_cfg"?"✕ Fechar":"✏️ Editar"}
          </button>
        </div>
        {editing!=="assistente_cfg"?(
          <>
            {[["Nome da assistente",asstNome],["Tom de atendimento",tomLabels[asstTom]],["Falar com humano","Sempre disponível nas conversas"]].map(([l,v])=>(
              <div key={l} className="config-row"><span className="config-label">{l}</span><span className="config-value">{v}</span></div>
            ))}
          </>
        ):(
          <>
            <div className="field"><label>Nome da assistente</label><input type="text" value={asstNome} onChange={e=>setAsstNome(e.target.value)} placeholder="Ex: Luna"/></div>
            <div className="field">
              <label>Tom de atendimento</label>
      ...
      <div className="dash-section" style={{marginBottom:16}}>
        <div className="dash-section-header">
          <span className="dash-section-title">Segurança — Alterar senha</span>
          <button className="dash-edit-btn" onClick={()=>setEditing(editing==="senha"?null:"senha")}>
            {editing==="senha"?"✕ Fechar":"✏️ Alterar"}
          </button>
        </div>
        {editing!=="senha"?(
          <div className="config-row"><span className="config-label">Senha</span><span className="config-value">••••••••</span></div>
        ):(
          <>
            <div className="field">
              <label>Senha atual</label>
              <div className="input-icon">
                <input type={showSenha?"text":"password"} value={senhaAtual} onChange={e=>setSenhaAtual(e.target.value)} placeholder="••••••••"/>
                <span className="icon" onClick={()=>setShowSenha(s=>!s)}>{showSenha?"🙈":"👁"}</span>
              </div>
            </div>
            <div className="field">
              ...
              {senhaNova&&<>
                ...
                <div className="strength-label" style={{color:stSenha.color}}>{stSenha.label}</div>
              </>}
            </div>
            <div className="field">
              <label>Confirmar nova senha</label>
              <input type="password" value={senhaConf} onChange={e=>setSenhaConf(e.target.value)} className={senhaConf&&senhaNova!==senhaConf?"error":""}/>
              {senhaConf&&senhaNova!==senhaConf&&<div className="field-error">As senhas não coincidem.</div>}
            </div>
            <div className="security-badge"><span>🔒</span> Armazenada com bcrypt — nunca em texto puro.</div>
            <div style={{display:"flex",gap:10,marginTop:16}}>
              <button className="btn-next primary" style={{padding:"10px 28px"}}
                disabled={!senhaAtual||!senhaNova||senhaNova!==senhaConf||stSenha.score<2}
                onClick={()=>{setSenhaAtual("");setSenhaNova("");setSenhaConf("");save("Senha");}}>
                Salvar nova senha
              </button>
              <button className="btn-back" onClick={()=>setEditing(null)}>Cancelar</button>
            </div>
      ...
      <div className="dash-section" style={{marginBottom:16}}>
        <div className="dash-section-header">
          <span className="dash-section-title">Plano e cobrança</span>
          <button className="dash-edit-btn" onClick={()=>setEditing(editing==="cobranca"?null:"cobranca")}>
            {editing==="cobranca"?"✕ Fechar":"✏️ Editar cartão"}
          </button>
        </div>
        {[["Plano atual","Profissional — R$ 197/mês"],["Próxima cobrança","06/07/2026"],["Cartão","•••• •••• •••• 4242"],["Status","Ativo — 14 dias de teste"]].map(([l,v])=>(
          <div key={l} className="config-row"><span className="config-label">{l}</span><span className="config-value">{v}</span></div>
        ))}
        {editing==="cobranca"&&(
          <div style={{borderTop:"1px solid rgba(0,0,0,.07)",paddingTop:16,marginTop:8}}>
            ...
            <div className="pci-badges" style={{marginBottom:16}}>
              <div className="pci-badge">🔒 SSL 256-bit</div>
              <div className="pci-badge">✅ PCI DSS</div>
              <div className="pci-badge">🛡 Tokenizado</div>
            </div>
            <div style={{display:"flex",gap:10}}>
      ...
      {/* ZONA DE RISCO */}
      <div className="dash-section" style={{border:"1px solid rgba(255,59,48,.2)"}}>
        <div className="dash-section-title" style={{color:"var(--red)",marginBottom:8}}>Zona de risco</div>
        <p style={{fontSize:14,color:"var(--muted)",marginBottom:14}}>Cancelar o plano encerrará os serviços da secretária ao fim do período pago. Você continuará com acesso até o final do mês.</p>
        <button style={{background:"none",border:"1px solid var(--red)",color:"var(--red)",borderRadius:100,padding:"8px 20px",fontSize:13,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:600}}
          onClick={()=>confirm("Tem certeza que deseja cancelar o plano?")&&alert("Plano cancelado. Você terá acesso até 06/07/2026.")}>
          Cancelar plano
        </button>
      </div>
    </>
